...
//				}
				...
				if (!m_objectIdClusters.get(index).isEmpty()) {
					...
					if (m_objectIdClusters.get(index).size() == 1) {
						...
						for (Integer nodeClusterId : clusteringObjects.getNodeClusterAndObjectInfo().keySet()) {
							for (int j = 0; j < clusteringObjects.getNodeClusterAndObjectInfo().get(nodeClusterId)
									.size(); j++) {
								if (clusteringObjects.getNodeClusterAndObjectInfo().get(nodeClusterId)
										.get(j) == index) {
									int startNode = m_nodeIdClusters.get(nodeClusterId).getFirst();
									System.out.println(startNode);
									int endNode = m_nodeIdClusters.get(nodeClusterId).getLast();
									System.out.println(endNode);

									if (gr.isTerminalNode(startNode)) {
										double dist1 = gr.getDistanceFromNodeToGivenObjOnSameEdge(endNode,
												m_objectIdClusters.get(index).getFirst());
										double dist2 = gr.getDistanceFromNodeToGivenObjOnSameEdge(endNode,
												m_objectIdClusters.get(index).getLast());
										if (dist1 > dist2) {
											int requiredQueryPoint = m_objectIdClusters.get(index).getLast();
											int redundantQueryPoint = m_objectIdClusters.get(index).getFirst();
											queriedObjCounter ++;
											int nearestFalseObject = nn.getNearestFalseObjectIdToGivenObjOnMap(m_graph,
													requiredQueryPoint);
											m_nearestNeighborSets.put(requiredQueryPoint, nearestFalseObject);
											m_nearestNeighborSets.put(redundantQueryPoint, nearestFalseObject);

										} else {
											int requiredQueryPoint = m_objectIdClusters.get(index).getFirst();
											int redundantQueryPoint = m_objectIdClusters.get(index).getLast();
											queriedObjCounter ++;
											int nearestFalseObject = nn.getNearestFalseObjectIdToGivenObjOnMap(m_graph,
													requiredQueryPoint);
											m_nearestNeighborSets.put(requiredQueryPoint, nearestFalseObject);
									...
									} else if (gr.isTerminalNode(endNode)) {
										double dist1 = gr.getDistanceFromNodeToGivenObjOnSameEdge(startNode,
												m_objectIdClusters.get(index).getFirst());
										double dist2 = gr.getDistanceFromNodeToGivenObjOnSameEdge(startNode,
												m_objectIdClusters.get(index).getLast());
										if (dist1 > dist2) {
											int requiredQueryPoint = m_objectIdClusters.get(index).getLast();
											int redundantQueryPoint = m_objectIdClusters.get(index).getFirst();
